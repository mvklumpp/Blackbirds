#*********************************************************************************
#Title----
#Stable immune function throughout moult progression regardless of age-specific moulting strategy in a European passerine

#Malin V. Klumpp & Arne Hegemann

#Script by M. V. Klumpp, October 2025
...
moult_ad <- moult_data %>%
  filter(ageF == "ad") %>%
  filter(!(moulting == "completed")) #remove the single individual with completed moult
...
#sex interaction
(plot_lysis_sex <- ggplot(moult_ad, aes(x = moulting, y = lysis_score)) +
  geom_boxplot(width = 0.5, fill="lightgrey")+
  geom_jitter(width = 0.1, size = 2, colour="grey56") +
  labs(y="Lysis (titre)", x="", title = "") + 
  facet_wrap(~sex_conclusion) +
  theme_classic()+
  theme_classic()+
  theme(plot.title = element_text(hjust = 0.5),
        axis.title = element_text(size = 28), 
